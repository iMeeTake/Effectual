...
public class MouthSteamParticleFactory implements ParticleProvider<SimpleParticleType> {
    ...
    static void spawn(double x, double y, double z, double dx, double dy, double dz, BreathAnchor anchor) {
        ...
        client.particleEngine.add(new MouthSteamParticle(level, x, y, z, dx, dy, dz, sprites, sprites.get(level.getRandom()), anchor));
    }
    ...
    public Particle createParticle(SimpleParticleType type, ClientLevel level, double x, double y, double z, double dx, double dy, double dz, RandomSource random) {
        return new MouthSteamParticle(level, x, y, z, dx, dy, dz, spriteSet, spriteSet.get(random), BreathAnchor.NONE);
    }
}
