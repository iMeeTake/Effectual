...
public class MudDustParticle extends RunDustParticle {
    ...
    public MudDustParticle(ClientLevel level, double x, double y, double z, double vx, double vy, double vz, TextureAtlasSprite sprite) {
        super(level, x, y, z, vx, vy, vz, Weight.HEAVY, 0.23F, 0.21F, 0.20F, sprite);
    }
    ...
    public static class Factory implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType type, ClientLevel level, double x, double y, double z, double dx, double dy, double dz, RandomSource random) {
            return new MudDustParticle(level, x, y, z, dx, dy, dz, this.spriteSet.get(random));
        }
    }
}