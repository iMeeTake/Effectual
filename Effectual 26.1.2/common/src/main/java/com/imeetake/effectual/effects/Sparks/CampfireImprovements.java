...
public class CampfireImprovements {
    ...
    public static void animateTick(Level level, BlockPos pos, RandomSource random, boolean isSoul) {
        if (!EffectualConfig.get().campfireImprovements) return;
        if (random.nextFloat() < 0.4f) {
            spark(pos, random, isSoul);
        }
    }

    private static void spark(BlockPos pos, RandomSource random, boolean isSoul) {
        ...
        SimpleParticleType particle = isSoul ? ModParticles.SOUL_SPARK.get() : ModParticles.SPARK.get();
        EffectualClientParticles.spawn(particle, x + ox, y + oy, z + oz, dx, dy, dz);
    }
}
