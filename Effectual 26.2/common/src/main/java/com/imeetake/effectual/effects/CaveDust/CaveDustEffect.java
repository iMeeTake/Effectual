...
public class CaveDustEffect {
    ...
    private static final Set<Block> ARTIFICIAL_BLOCKS = buildArtificialBlocks();

    private static Set<Block> buildArtificialBlocks() {
        Set<Block> blocks = new HashSet<>(Set.of(
                ...
                Blocks.IRON_BARS, Blocks.IRON_CHAIN,
        ...
        for (WeatheringCopperCollection<Block> copper : List.of(
                Blocks.COPPER_BLOCK, Blocks.CUT_COPPER, Blocks.CHISELED_COPPER,
                Blocks.COPPER_GRATE, Blocks.COPPER_BULB, Blocks.COPPER_DOOR, Blocks.COPPER_TRAPDOOR)) {
            blocks.addAll(copper.asList());
        }
        return blocks;
    }
    ...
    private static void spawnDustParticles(Level level, BlockPos center) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            EffectualClientParticles.spawn(ParticleTypes.WHITE_ASH,
                    x, y, z,
                    0, -0.01, 0);
        }
    }
